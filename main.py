...
# ==========================================
    ...
    if df_s.empty: st.warning("등록된 학생이 없습니다.")
        ...
        if s_sel:
            ...
            target_ym = f"{st.session_state.view_year}-{st.session_state.view_month:02d}"
            month_att = df_a[df_a.iloc[:,0].astype(str).str.startswith(target_ym)] if not df_a.empty else pd.DataFrame()
            
            # 1. 이달의 모든 학생 자습 시간 합산
            self_study_totals = {}
            if not month_att.empty:
                for _, r in month_att.iterrows():
                    sn = str(r.iloc[2])
                    memo = str(r.get('비고', ''))
                    if '자습:' in memo:
                        try:
                            mins = int(memo.split('자습:')[1].split()[0])
